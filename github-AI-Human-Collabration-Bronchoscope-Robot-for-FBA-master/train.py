...
from model import HighLevelCIL, SingleLevelCIL, SingleLevelCILMultiFrame, fixedBranchedCIMNetWithDepthAngle, ResnetGenerator_our
from onlineSimulation_with_depth_transfer_angle_disctrl_network_multiframe_levelcenterline_longrange_dyn import onlineSimulationWithNetwork
from dataset import AlignDataSetDaggerHighLevel, AlignDataSetDaggerSingleLevel, AlignDataSetDaggerSingleLevelMultiFrame, AlignDataSetDaggerSingleLevelMultiFrameFromStart
...
def get_args():
    ...
    parser.add_argument('-b', '--batch-size', dest='batchsize', metavar='B', type=int, default=100, help='Batch size')
    ...
    parser.add_argument('-fe', '--load-epoch', dest='load_epoch', type=int, default=169, help='Load model from which epoch')
    ...
    parser.add_argument('--history_length', dest='history_length', type=int, default=10, help='History length')
    parser.add_argument('--low-level-load-epoch', dest='low_level_load_epoch', type=int, default=269, help='Load model from which epoch')
    parser.add_argument('--low-level-model-dir', dest='low_level_model_dir', type=str, default="F:/conditional_imitation_learning_checkpoints/sliliconmodel1&2_pink_hypercontrol(10-30)_randstart_CILRS_fixebranched_fixcor_aug70_transfer30_with_depth_angle_advd_distctrl_randroll_randintensity", help='Path of trained model for saving')
...
if __name__ == '__main__':
    ...
    if args.load:
        ...
        args.load_epoch = -1
    # pretrained_dict = torch.load("checkpoints/silicon1&2_sample10_levelcenterline/regular_229.pth", map_location=device)
    # model_dict = net.state_dict()
    # pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}  # 不必要的键去除掉
    # model_dict.update(pretrained_dict)  # 覆盖现有的字典里的条目
    # net.load_state_dict(model_dict)
    ...
    # low_level_net = fixedBranchedCIMNetWithDepthAngle()
    # pretrained_dict = torch.load(os.path.join(args.low_level_model_dir, "regular_{}.pth".format(args.low_level_load_epoch)), map_location=device)
    # model_dict = low_level_net.state_dict()
    # pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}  # 不必要的键去除掉
    # model_dict.update(pretrained_dict)  # 覆盖现有的字典里的条目
    # low_level_net.load_state_dict(model_dict)
    # low_level_net.to(device=device)
    # low_level_net.eval()
    ...
    dataset = AlignDataSetDaggerSingleLevelMultiFrameFromStart(args.dataset_dir, args.history_length, train_flag=True)
    ...
    for epoch in range(args.load_epoch + 1, args.epochs + args.decay_epochs + 1):
        ...
        for batch in train_loader:
            ...
            input = batch[0].to(device=device, dtype=torch.float32)
            action = batch[1].to(device=device, dtype=torch.float32)
            stop = batch[2].to(device=device, dtype=torch.long)
            predicted_action, predicted_stop = net(input)
            ...
            loss_action = L1_loss(action * 100, predicted_action * (np.pi / 2) * 100)
            loss_stop = ce_loss(predicted_stop, stop)  # scale value 100 for fast convergence, pred action belongs to (-1, 1) and needs scaling to (-pi / 2, pi / 2)
            loss = loss_action + loss_stop * 10
            # g = make_dot(output)
            # g.view()
            ...
            if args.tensorboard:
                ...
                writer.add_scalar('loss/stop loss', loss_stop.item(), global_step=global_batch_count)
        ...
        if (epoch + 1) % 1 == 0:  # online validation and update
            with torch.no_grad():
                ...
                args.transfer_model_dir = "/home/liulilu/Data1/attention_gan_with_depth/checkpoints/bronchus14_attentiongan_AtoB_add_depth2/{}_net_G_A.pth".format(random_index)
                pretrained_dict = torch.load(args.transfer_model_dir, map_location=device)
                model_dict = net_transfer.state_dict()
                pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}  # 不必要的键去除掉
                model_dict.update(pretrained_dict)  # 覆盖现有的字典里的条目
                ...
                while dataset_size == len(dataset):
                    ...
                    simulator = onlineSimulationWithNetwork(args.dataset_dir, args.history_length, online_test_centerline_name, renderer='pyrender')
                    simulator.run(net, epoch=epoch, net_transfer=net_transfer, transform_func=dataset.transforms_eval, transform_func_transfer=transform_func_transfer)
                    # if args.tensorboard:
                    #     for index, error in enumerate(path_centerline_error_list):
